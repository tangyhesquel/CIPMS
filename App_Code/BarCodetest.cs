...
namespace Code
{
    public class BarCode
    {
        public class Code128
        {
            ...
            /// <returns>图形</returns>
            public Bitmap GetCodeImage(string p_Text, Encode p_Code, int magnify)
            {
                ...
                _Text += "2331112"; //结束位
                Bitmap _CodeImage = GetImage(_Text, magnify);
            ...
            /// <param name="p_Text">文字</param>
            /// <returns>图形</returns>
            private Bitmap GetImage(string p_Text, int magnify)
            {
                char[] _Value = p_Text.ToCharArray();
                int _Width = 0;
                for (int i = 0; i != _Value.Length; i++)
                {
                    _Width += Int32.Parse(_Value[i].ToString()) * (m_Magnify + 1)*magnify;
                }
                ...
                for (int i = 0; i != _Value.Length; i++)
                {
                    int _ValueNumb = Int32.Parse(_Value[i].ToString()) * (m_Magnify + 1)*magnify;  //获取宽和放大系数
                    if (!((i & 1) == 0))
                    {
